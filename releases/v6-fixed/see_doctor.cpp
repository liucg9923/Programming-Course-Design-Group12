...
static void modifyRecordByRevoke(void) {
     ...
     * 挂号/住院/出院结算/充值等业务记录应通过其他入口处理。 */
    /* V5 修复 Bug 21：VISIT 记录由 EXAM/PRESCRIBE 衍生，原始 VISIT 仅含诊断
     * 文本无金额无药品，更正方式应为"修改诊断"而非"修改药品"。
     * 但原代码允许 VISIT 进入更正流程，却没有为 VISIT 写"录入新数据"分支，
     * 导致 newRec memcpy 后保留旧 medId/medCount/total，且用户输入的两次
     * 数字会污染下一次 GetSafeInt() 调用。
     * 这里收紧：VISIT 走专用快速分支（仅修改诊断），不混入药品/费用询问。 */
    if (oldRec->type == REC_TYPE_VISIT) {
        ...
        /* 新建一条与旧记录完全等价但诊断改变的记录，旧记录红冲。 */
        {
            RecordNode* newRec = (RecordNode*)malloc(sizeof(RecordNode));
            if (!newRec) {
                printf("内存分配失败。\n");
                return;
            }
            ...
            strncpy(newRec->diagnosis, newDiag, MAX_DIAG_LEN - 1);
            newRec->diagnosis[MAX_DIAG_LEN - 1] = '\0';
            ...
            printf("更正完成：\n");
            printf("  旧记录（已红冲）：%s\n", oldRec->recordId);
            printf("  新记录：%s 诊断=%s\n", newRec->recordId, newRec->diagnosis);
        }
        return;
    }
